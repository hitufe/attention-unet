...
def get_data_training(DRIVE_train_imgs_original,
    ...
    # patches_masks_train=np.transpose(patches_masks_train, (0, 3, 2, 1))
...
def get_data_testing_overlap(DRIVE_test_imgs_original, DRIVE_test_groudTruth, Imgs_to_test, patch_height, patch_width, stride_height, stride_width, mode="qwer"):
    ### test
    test_imgs_original = load_hdf5(DRIVE_test_imgs_original)
    test_masks = load_hdf5(DRIVE_test_groudTruth)
    if mode == "red":
        test_imgs = my_PreProc(test_imgs_original, "red")
    elif mode == "green":
        test_imgs = my_PreProc(test_imgs_original, "green")
    elif mode == "blue":
        test_imgs = my_PreProc(test_imgs_original, "blue")
    else:
        test_imgs = my_PreProc(test_imgs_original)
    test_masks = test_masks/255.
    #extend both images and masks so they can be divided exactly by the patches dimensions
    ...
    # patches_imgs_test = np.transpose(patches_imgs_test, (0, 3, 2, 1))
    ...
    for i in range(full_imgs.shape[0]):  #loop over the full images
        for h in range((img_h-patch_h)//stride_h+1):
            for w in range((img_w-patch_w)//stride_w+1):
                ...
                patches[iter_tot]=patch
                # print('patchessize', patches.shape)
                iter_tot +=1   #total
    assert (iter_tot==N_patches_tot)
    return patches  #array with all the full_imgs divided in patches
